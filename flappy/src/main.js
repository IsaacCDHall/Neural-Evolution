...
new P5((p5) => {
    ...
    // changes from WEBGL background so theres no sloMO start
    tf.setBackend('cpu');
    p5.setup = () => {
        ...
        for (var i = 0; i < 100; i++) {
            ...
            pipe.text = '#' + (i + 1);
            pipes.push(pipe);
            //-- makes this too difficult, anything above 1 breaks and below .5 too easy
            spacing-= .7;
        }
        statesP = p5.createP();
    ...
    p5.draw = () => {
        ...
        if (pop.birds.every(b => !b.alive) && (!player || !player.alive)) {
            pop.calcFitness();
            console.log('fitness:', pop.birds.sort((a, b) => b.pos.x - a.pos.x)[0].dna.values);
        ...
        html += `High Score: ${stats.bestEver}<br>`;
        html += `Population: ${stats.birds}<br>`;
